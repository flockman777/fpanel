...
import { FolderLock, KeyRound, Plus, Trash2 } from "lucide-react";
...
export default function Ftp() {
  ...
  const load = async () => {
    try {
      setRows(await api<Row[]>("/client/ftp"));
    } catch (e: any) {
  ...
  const create = async (e: React.FormEvent) => {
    ...
    setBusy(true);
    try {
      await api<Row>("/client/ftp", {
        method: "POST",
        body: JSON.stringify({ username, password, directory: directory.trim() || "public_html" }),
      });
      setOpen(false);
      setUsername("");
      setPassword("");
  ...
  const remove = async (r: Row) => {
    ...
    try {
      await api(`/client/ftp/${r.id}`, { method: "DELETE" });
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-gray-500">Manage FTP users that can access your files</p>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <form onSubmit={create} className="w-full max-w-md rounded-xl bg-white p-6 shadow-xl">
            ...
            <div className="mb-3">
              ...
              <p className="mt-1 text-[11px] text-gray-400">Relative to your home, e.g. public_html</p>
            </div>
