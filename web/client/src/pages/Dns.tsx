...
export default function Dns() {
  ...
  const loadAll = async () => {
    try {
      const [d, r] = await Promise.all([
        ...
        api<Record[]>("/client/dns"),
      ]);
      setDomains(d);
      if (domainId && r.some((rec) => rec.domain_id === Number(domainId))) {
        setRecords(r.filter((rec) => rec.domain_id === Number(domainId)));
      } else {
        setRecords(r);
      }
    } catch (e: any) {
  ...
  const loadRecords = async () => {
    try {
      const r = await api<Record[]>("/client/dns");
      setRecords(domainId ? r.filter((rec) => rec.domain_id === Number(domainId)) : r);
    } catch (e: any) {
      notify(String(e.message || e), "err");
    }
  };
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      if (editing) {
        await api(`/client/dns/${editing.id}`, { method: "PUT", body: JSON.stringify(body) });
        notify("Record updated");
      } else {
        await api("/client/dns", { method: "POST", body: JSON.stringify(body) });
        notify("Record added");
      }
  ...
  const shown = records.filter((r) => r.domain_id === Number(domainId) || !domainId);
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-gray-500">Manage the DNS records of your domains</p>
      </div>
            ...
            {shown.length === 0 ? (
              <tr>
                <td colSpan={6} className="px-5 py-10 text-center text-gray-400">
                  No DNS records yet. Select a domain and add your first record.
                </td>
              </tr>
            ) : (
              shown.map((r) => (
                <tr key={r.id} className="hover:bg-gray-50">
                  <td className="px-5 py-3 text-xs text-gray-500">{r.domain}</td>
                  ...
                  <td className="max-w-[26rem] truncate px-5 py-3 font-mono text-xs text-gray-600" title={r.value}>
                    {r.value}
                  </td>
