import { Flame, RotateCcw, Save } from "lucide-react";
...
export default function Waf() {
  ...
  const load = async () => {
    try {
      setRows(await api<Row[]>("/client/waf"));
  ...
  const remove = async (row: Row) => {
    ...
    try {
      await api(`/client/waf/${row.id}`, { method: "DELETE" });
      notify("WAF disabled");
      load();
    } catch (e: any) {
      notify(String(e.message || e), "err");
  ...
  const toggleRule = (i: number, ruleId: string) =>
    setRows(
      rows.map((r, idx) =>
        idx === i
          ? { ...r, rules: r.rules.map((rule) => (rule.id === ruleId ? { ...rule, action: rule.action === "block" ? "ignore" : "block" } : rule)) }
          : r
      )
    );
          ...
          <div className="flex flex-wrap gap-2">
            {notProtected.map((d) => (
              <button
                ...
                onClick={async () => {
                  try {
                    const rules = await api<WafRule[]>("/client/waf/defaults");
                    const r = await api<Row>("/client/waf", {
                      method: "PUT",
                      body: JSON.stringify({ domain_id: d.id, enabled: true, mode: "block", rules }),
                    });
                    setRows([r, ...rows]);
              ...
              <div key={row.domain_id} className="rounded-lg border border-gray-200 p-4">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <select value={row.mode} onChange={(e) => setRow(i, { mode: e.target.value })} className="rounded-lg border border-gray-300 px-2 py-1.5 text-xs">
                ...
                <button onClick={() => setExpanded(expanded === row.domain_id ? null : row.domain_id)} className="mt-3 text-xs font-medium text-brand-700 hover:underline">
