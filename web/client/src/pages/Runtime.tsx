...
export default function Runtime() {
  ...
  const load = async () => {
    try {
      const r = await api<ListResp>("/client/runtime");
      setRows(r.apps);
  ...
  useEffect(() => {
    ...
    api<Domain[]>("/client/domains")
      .then((d) => {
        setDomains(d);
        if (d[0]) setDomainId(String(d[0].id));
      })
  ...
  const act = async (app: RunApp, action: string) => {
    try {
      await api(`/client/runtime/${app.id}/${action}`, { method: "POST" });
  ...
  const drop = async (app: RunApp) => {
    ...
    try {
      await api(`/client/runtime/${app.id}`, { method: "DELETE" });
      notify("App deleted");
      load();
    } catch (e: any) {
      notify(String(e.message || e), "err");
  ...
  const openLog = async (app: RunApp) => {
    ...
    setLogLines([]);
    try {
      const r = await api<{ lines: string[] }>(`/client/runtime/${app.id}/log?lines=200`);
      setLogLines(r.lines);
    } catch (e: any) {
      notify(String(e.message || e), "err");
    }
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-gray-500">
          Run a Node.js, Python, PHP or other app on your domain with auto-restart
        </p>
      </div>
      ...
      <section className="rounded-xl border border-gray-200 bg-white p-5">
        <div className="mb-3 flex items-center justify-between">
          <span className="font-semibold text-gray-800">Runtime Apps ({rows.length})</span>
        ...
        {showForm && (
          <form onSubmit={create} className="mb-5 rounded-lg border border-brand-200 bg-brand-50 p-4">
            <div className="grid grid-cols-2 gap-4 md:grid-cols-3">
              ...
              <div>
                <label className="mb-1 block text-xs font-medium text-gray-600">Env vars (JSON, optional)</label>
                <input value={env} onChange={(e) => setEnv(e.target.value)} className={base} placeholder='{"NODE_ENV":"production"}' />
              </div>
            </div>
            <div className="mt-4 flex items-center justify-between">
              <p className="text-xs text-gray-500">
                Entry point runs from your document root. No need to hardcode the port — the{" "}
                <code className="rounded bg-gray-100 px-1">PORT</code> environment variable is set for you.
              </p>
        ...
        {rows.length === 0 ? (
          <p className="text-sm text-gray-500">No runtime apps yet.</p>
        ) : (
            ...
            <table className="w-full text-left text-sm">
              <thead>
                <tr className="border-b border-gray-200 text-xs uppercase tracking-wider text-gray-500">
                  ...
                  <th className="px-3 py-2">Auto-restart</th>
              ...
              <tbody>
                {rows.map((a) => (
                  <tr key={a.id} className="border-b border-gray-100">
                    ...
                    <td className="px-3 py-2.5 text-gray-600">{a.auto_restart ? "Yes" : "No"}</td>
                    <td className="px-3 py-2.5">
