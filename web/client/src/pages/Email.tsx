...
export default function Email() {
  ...
  const [pwd, setPwd] = useState<Account | null>(null);
  ...
  const load = async () => {
    try {
      const [d, a, f, r, df] = await Promise.all([
        api<Domain[]>("/client/domains"),
        api<Account[]>("/client/email/accounts"),
        api<Forwarder[]>("/client/email/forwarders"),
        api<AutoResp[]>("/client/email/autoresponders"),
        api<DefaultRow[]>("/client/email/default"),
      ]);
      setDomains(d);
      setAccounts(a);
      setForwarders(f);
  ...
  const createAccount = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await api("/client/email/accounts", {
        method: "POST",
        body: JSON.stringify({
  ...
  const dropAccount = async (a: Account) => {
    ...
    try {
      await api(`/client/email/accounts/${a.id}`, { method: "DELETE" });
  ...
  const changePwd = async (e: React.FormEvent) => {
    ...
    if (!pwd) return;
    try {
      await api(`/client/email/accounts/${pwd.id}/password`, {
        method: "POST",
  ...
  const createForwarder = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await api("/client/email/forwarders", {
        method: "POST",
        body: JSON.stringify({
  ...
  const dropForwarder = async (f: Forwarder) => {
    ...
    try {
      await api(`/client/email/forwarders/${f.id}`, { method: "DELETE" });
  ...
  const createResp = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await api("/client/email/autoresponders", {
        method: "POST",
        body: JSON.stringify({
  ...
  const dropResp = async (r: AutoResp) => {
    ...
    try {
      await api(`/client/email/autoresponders/${r.id}`, { method: "DELETE" });
  ...
  const saveDefault = async (d: Domain) => {
    ...
    try {
      await api("/client/email/default", {
        method: "POST",
        body: JSON.stringify({
          domain_id: d.id,
          action: v.action,
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-gray-500">
          Email accounts, forwarders and autoresponders for your domains
        </p>
      </div>
          ...
          <section className="rounded-xl border border-gray-200 bg-white p-5">
            <div className="mb-3 flex items-center justify-between">
              <div className="flex items-center gap-2 text-gray-800">
                ...
                <span className="font-semibold">Email Accounts ({accounts.length})</span>
              </div>
            </div>
            {accounts.length === 0 ? (
