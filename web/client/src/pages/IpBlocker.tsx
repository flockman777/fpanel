...
export default function IpBlocker() {
  ...
  const load = async () => {
    try {
      setRows(await api<Row[]>("/client/ipblocker"));
  ...
  const add = async () => {
    if (!ip.trim()) return notify("IP address is required", "err");
    setBusy(true);
    try {
      await api("/client/ipblocker", {
        method: "POST",
        body: JSON.stringify({ domain_id: domainId ? Number(domainId) : null, ip: ip.trim(), reason: reason.trim() || null }),
      });
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-gray-500">Block client IP addresses from reaching your domains</p>
      </div>
