...
import { Boxes, CheckCircle2, ExternalLink, PackageOpen, Plus, RefreshCw, Trash2 } from "lucide-react";
...
export default function Software() {
  ...
  const load = async () => {
    try {
      const r = await api<AppsResp>("/client/apps");
      setRows(r.rows);
      setTools(r.tools);
  ...
  const install = async (e: React.FormEvent) => {
    ...
    setBusy(true);
    try {
      const row = await api<AppRow>("/client/apps", {
        method: "POST",
        body: JSON.stringify({
  ...
  const upgrade = async () => {
    ...
    setUpBusy(true);
    try {
      await api(`/client/apps/${upRow.id}/upgrade`, {
        method: "POST",
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">
            Install popular applications on your domain with their own MySQL database
          </p>
        </div>
      ...
      <section className="rounded-xl border border-gray-200 bg-white p-5">
        ...
        {rows.length === 0 ? (
          <p className="text-sm text-gray-500">Nothing installed yet. Choose WordPress, Laravel or OJS above.</p>
        ) : (
