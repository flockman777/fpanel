...
export default function Cron() {
  ...
  const [form, setForm] = useState({ domain_id: "", preset: "", schedule: "", command: "", description: "" });
  ...
  const load = async () => {
    try {
      const [j, d] = await Promise.all([api<CronJob[]>("/client/cron"), api<Domain[]>("/client/domains")]);
      setJobs(j);
      setDomains(d);
    } catch (e: any) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setShowForm(false);
      setForm({ domain_id: "", preset: "", schedule: "", command: "", description: "" });
      load();
    } catch (e: any) {
      notify(String(e.message || e), "err");
    } finally {
  ...
  const remove = async (job: CronJob) => {
    if (!await askConfirm("Delete this cron job?")) return;
    try {
      await api(`/client/cron/${job.id}`, { method: "DELETE" });
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">Schedule commands to run on your account</p>
        </div>
            ...
            {jobs.length === 0 ? (
              ...
              jobs.map((j) => (
                <tr key={j.id} className="hover:bg-gray-50">
                  ...
                  <td className="max-w-[22rem] truncate px-5 py-3 text-xs text-gray-700" title={`${j.command}\n${j.description || ""}`}>
