...
export default function Ssh() {
  ...
  const load = async () => {
    if (!accountId) return;
    try {
      setRows(await api<Row[]>(`/ssh?account_id=${accountId}`));
    } catch (e: any) {
  ...
  const create = async (e: React.FormEvent) => {
    ...
    setBusy(true);
    try {
      const res = await api<{ user: Row; private_key: string | null }>("/ssh", {
        method: "POST",
        body: JSON.stringify({
          account_id: Number(accountId),
          username,
          auth_type: authType,
          authorized_keys: authorizedKeys.trim() || null,
        }),
      });
      setOpen(false);
      setUsername("");
      setAuthorizedKeys("");
      setPrivateKey(res.private_key ? { user: res.user.username, key: res.private_key } : null);
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-gray-500">Manage SSH users and SSH keys attached to this account</p>
      </div>
          ...
          <form onSubmit={create} className="w-full max-w-md rounded-xl bg-white p-6 shadow-xl">
            ...
            <div className="mb-3">
              <label className="mb-1 block text-xs font-medium text-gray-600">
                {authType === "key" ? "Authorized keys (leave empty to auto-generate)" : "Authorized keys (SSH command tagging, optional)"}
              </label>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-lg rounded-xl bg-white p-6 shadow-xl">
            ...
            <p className="mb-3 text-xs text-amber-700">
              Copy this private key now. It is only shown once and is not stored on the server.
            </p>
