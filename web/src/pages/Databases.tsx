...
export default function Databases() {
  ...
  const [dbs, setDbs] = useState<DatabaseAdmin[]>([]);
  const [users, setUsers] = useState<DbUserAdmin[]>([]);
  ...
  const [manage, setManage] = useState<DatabaseAdmin | null>(null);
  const [grants, setGrants] = useState<Privilege[]>([]);
  const [edit, setEdit] = useState<{ d: DatabaseAdmin; bu: BoundUser; privs: string[] } | null>(null);
  ...
  const load = async () => {
    ...
    try {
      const [d, u] = await Promise.all([
        api<DatabaseAdmin[]>(`/databases?account_id=${accountId}`),
        api<DbUserAdmin[]>(`/databases/db-users?account_id=${accountId}`),
      ]);
      setDbs(d);
      setUsers(u);
    } catch (e: any) {
  ...
  const createDb = async (e: React.FormEvent) => {
    ...
    try {
      await api("/databases", {
        method: "POST",
        body: JSON.stringify({ name: dbName.trim(), account_id: Number(accountId) }),
      });
      setDbName("");
      setShowDbForm(false);
  ...
  const createUser = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await api("/databases/db-users", {
        method: "POST",
        body: JSON.stringify({ username: uname.trim(), password: pass, account_id: Number(accountId) }),
      });
      setUname("");
      setPass("");
  ...
  const openManage = async (d: DatabaseAdmin) => {
    setManage(d);
    setAddUserId("");
    setAddPrivs(["ALL PRIVILEGES"]);
    try {
      const g = await api<Privilege[]>(
        `/databases/db-privileges?account_id=${accountId}&db_id=${d.id}`
      );
      setGrants(g);
    } catch (e: any) {
      setGrants([]);
  ...
  const addUser = async (e: React.FormEvent) => {
    ...
    try {
      await api("/databases/db-privileges", {
        method: "POST",
        body: JSON.stringify({
          db_id: Number(addDbId),
  ...
  const saveEdit = async (e: React.FormEvent) => {
    ...
    try {
      await api("/databases/db-privileges", {
        ...
        body: JSON.stringify({
          db_id: edit.d.id,
          user_id: edit.bu.user_id,
          privileges: edit.privs,
          account_id: Number(accountId),
        }),
      });
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">
            Manage databases, users and privileges for hosting accounts
          </p>
        </div>
        <select
          value={accountId}
      ...
      {acc && (
        <div className="rounded-xl border border-brand-200 bg-brand-50 p-5">
          <div className="mb-2 flex items-center gap-2 text-brand-700">
            ...
            <span className="font-semibold">Connection details for {acc.username}</span>
          </div>
          <div className="grid grid-cols-1 gap-2 text-sm text-gray-700 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <div>
              ...
              <code className="rounded bg-white/70 px-1.5 py-0.5">{acc.username}_</code>
            </div>
            <div>
      ...
      <section className="rounded-xl border border-gray-200 bg-white shadow-sm">
        <div className="flex items-center justify-between border-b border-gray-100 px-5 py-4">
          ...
          <button onClick={() => setShowDbForm(!showDbForm)} className={btn} disabled={!accountId}>
        ...
        {showDbForm && acc && (
          <form onSubmit={createDb} className="border-b border-gray-100 bg-brand-50/50 p-5">
            <div className="flex flex-wrap items-end gap-3">
              <span className="pb-2.5 text-sm font-medium text-gray-500">{acc.username}_</span>
              <input
                value={dbName}
            ...
            {dbs.length === 0 ? (
              <tr>
                <td colSpan={4} className="px-5 py-10 text-center text-gray-400">
                  No databases for this account yet.
                </td>
              </tr>
            ) : (
              dbs.map((d) => (
      ...
      <section className="rounded-xl border border-gray-200 bg-white shadow-sm">
        <div className="flex items-center justify-between border-b border-gray-100 px-5 py-4">
          ...
          <button onClick={() => setShowUserForm(!showUserForm)} className={btn} disabled={!accountId}>
        ...
        <table className="w-full text-left text-sm">
          ...
          <tbody className="divide-y divide-gray-100">
            {users.length === 0 ? (
              <tr>
                <td colSpan={4} className="px-5 py-10 text-center text-gray-400">
                  No MySQL users for this account yet.
                </td>
              </tr>
            ) : (
              users.map((u) => (
                  ...
                  >
                    <div>
                      <div className="flex items-center gap-2">
                        <span className="text-sm font-medium text-gray-800">{g.username}</span>
