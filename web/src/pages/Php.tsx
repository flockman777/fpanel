...
export default function Php() {
  ...
  const load = async () => {
    if (!accountId) return;
    try {
      const r = await api<PhpResp>(`/php?account_id=${accountId}`);
      setRows(r.rows);
  ...
  const save = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await api(`/php?account_id=${accountId}`, {
        method: "PUT",
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-gray-500">PHP version and directives per domain, across accounts</p>
      </div>
      ...
      <div className="flex items-center gap-2 rounded-xl border border-gray-200 bg-white px-4 py-3">
        ...
        <span className="text-xs text-gray-400">PHP versions: {versions.join(", ")}</span>
      </div>
                  ...
                  <tr key={r.domain_id} className="border-b border-gray-100">
                    ...
                    <td className="px-3 py-2.5">
                      <span className={`rounded-full px-2.5 py-1 text-xs font-medium ${r.version === "system" ? "bg-gray-100 text-gray-500" : "bg-indigo-50 text-indigo-700"}`}>
                        {r.version === "system" ? "Server default" : "PHP " + r.version}
                      </span>
                    </td>
            ...
            <div className="mt-4 space-y-3">
              ...
              {INI_DEFS.map((d) => (
                <div key={d.key} className="grid grid-cols-2 items-center gap-3">
                  ...
                  {d.type === "bool" ? (
                    <select value={ini[d.key] ?? ""} onChange={(e) => setIni({ ...ini, [d.key]: e.target.value })} className={base}>
                      <option value="">Server default</option>
                      <option value="On">On</option>
                      <option value="Off">Off</option>
                    </select>
                  ) : (
                    <input value={ini[d.key] ?? ""} onChange={(e) => setIni({ ...ini, [d.key]: e.target.value })} className={base} placeholder={d.type === "size" ? d.def : d.def} />
                  )}
                </div>
              ))}
            </div>
